...
if ($_SERVER['REQUEST_METHOD'] === 'POST')
{
	...
	$data = json_decode(htmlspecialchars_decode ($tmp));
		...
		if(MODULE_PAYMENT_EXPRESSPAYEPOS_USESIGNATUREFORNOTIF == 'True')
		{
			// Секретное слово указывается в настройках личного кабинета
			$secretWord = MODULE_PAYMENT_EXPRESSPAYEPOS_SECRETWORDFORNOTIF;
...
{
	$sql_data_array = array('orders_status' => MODULE_PAYMENT_EXPRESSPAYEPOS_ORDER_STATUS_ID);
	vam_db_perform('orders', $sql_data_array, 'update', "orders_id='".$data->AccountNo."'");
  
	$sql_data_arrax = array('orders_id' => $data->AccountNo,
							'orders_status_id' => MODULE_PAYMENT_EXPRESSPAYEPOS_ORDER_STATUS_ID,
							'date_added' => 'now()',
							'customer_notified' => '0',
	...
	vam_send_answer_template($data->AccountNo,MODULE_PAYMENT_EXPRESSPAYEPOS_ORDER_STATUS_ID,'on','on');
