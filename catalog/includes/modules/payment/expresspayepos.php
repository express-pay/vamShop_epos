<?php
/* -----------------------------------------------------------------------------------------
   $Id: expresspayepos.php 998 2012/09/16 13:24:46 VaM $
  ...
  class expresspayepos{
    var $code, $title, $description, $enabled;

// class constructor
    function expresspayepos() {
      global $order;

      $this->code = 'expresspayepos';
      $this->title = MODULE_PAYMENT_EXPRESSPAYEPOS_TEXT_TITLE;
      $this->public_title = MODULE_PAYMENT_EXPRESSPAYEPOS_TEXT_PUBLIC_TITLE;
      $this->description = MODULE_PAYMENT_EXPRESSPAYEPOS_TEXT_ADMIN_DESCRIPTION;
      $this->icon = "https://express-pay.by/Media/Images/Logo.png";
      $this->sort_order = MODULE_PAYMENT_EXPRESSPAYEPOS_SORT_ORDER;
      $this->enabled = ((MODULE_PAYMENT_EXPRESSPAYEPOS_STATUS == 'True') ? true : false);

      ((MODULE_PAYMENT_EXPRESSPAYEPOS_ISTEST == 'True') ? $this->form_action_url = 'https://sandbox-api.express-pay.by/v1/web_invoices' : $this->form_action_url = 'https://api.express-pay.by/v1/web_invoices');
    }
...
// class methods
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_EXPRESSPAYEPOS_ZONE > 0) ) {
        $check_flag = false;
        $check_query = vam_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_EXPRESSPAYEPOS_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function selection() {
      ...
      if (isset($_SESSION['cart_expresspayepos_id'])) {
        $order_id = substr($_SESSION['cart_expresspayepos_id'], strpos($_SESSION['cart_expresspayepos_id'], '-')+1);
        ...
        if (vam_db_num_rows($check_query) < 1) {
          ...
          unset($_SESSION['cart_expresspayepos_id']);
        }
      }
    ...
    function confirmation() {
      ...
      if (isset($_SESSION['cartID'])) {
        ...
        if (isset($_SESSION['cart_expresspayepos_id'])) {
          ...
          if ( ($curr['currency'] != $order->info['currency']) || ($_SESSION['cartID'] != substr($_SESSION['cart_expresspayepos_id'], 0, strlen($_SESSION['cartID']))) ) {
            $check_query = vam_db_query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int)$order_id . '" limit 1');
...
//PIN				
          ...
          $_SESSION['cart_expresspayepos_id'] = $_SESSION['cartID'] . '-' . $insert_id;
        }
      }

      return array('title' => MODULE_PAYMENT_EXPRESSPAYEPOS_TEXT_DESCRIPTION);
    }
    ...
    function process_button() {
      ...
      $process_button_string = '';
			...
			#номер заказа
			$order_no = substr($_SESSION['cart_expresspayepos_id'], strpos($_SESSION['cart_expresspayepos_id'], '-')+1);

			#получаем стоимость заказа в белорусских рублях
      $sum = number_format($order->info["total"], 2, ',', '');
      
      $request_params = array(
        "ServiceId" => MODULE_PAYMENT_EXPRESSPAYEPOS_SERVICEID,
        "AccountNo" => $order_no,
        "Amount" => $sum,
        "Currency" => 933,
        ...
        "IsNameEditable" => MODULE_PAYMENT_EXPRESSPAYEPOS_ISNAMEEDIT ? '1' : '0',
        "IsAddressEditable" => MODULE_PAYMENT_EXPRESSPAYEPOS_ISADRESSEDIT ? '1' : '0',
        "IsAmountEditable" => MODULE_PAYMENT_EXPRESSPAYEPOS_ISAMOUNTEDIT ? '1' : '0',
        "ReturnType" => "redirect",
        "ReturnUrl" => vam_href_link('expresspayepos_result.php?param=success&amount='. $sum),
        "FailUrl" => vam_href_link('expresspayepos_result.php?param=cancel')
      );

      $secret_word = MODULE_PAYMENT_EXPRESSPAYEPOS_USESIGNATURE == "True" ? MODULE_PAYMENT_EXPRESSPAYEPOS_SECRETWORD : "";

      $request_params['Signature'] = $this->compute_signature($request_params, $secret_word, MODULE_PAYMENT_EXPRESSPAYEPOS_TOKEN, 'add_invoice');
...
// unregister session variables used during checkout
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = vam_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_EXPRESSPAYEPOS_STATUS'");
    ...
    {
      return str_replace('/admin', null, vam_href_link('expresspayepos_notify.php'));
    }

    function install() {
      $gg = $this->getNotifUrl();
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_EXPRESSPAYEPOS_STATUS', 'True', '6', '1', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_EXPRESSPAYEPOS_ALLOWED', '', '6', '2', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_EXPRESSPAYEPOS_SORT_ORDER', '0', '6', '3', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_PAYMENT_EXPRESSPAYEPOS_ZONE', '0', '6', '4', 'vam_get_zone_class_title', 'vam_cfg_pull_down_zone_classes(', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_EXPRESSPAYEPOS_ORDER_STATUS_ID', '0', '6', '5', 'vam_cfg_pull_down_order_statuses(', 'vam_get_order_status_name', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_EXPRESSPAYEPOS_ISTEST', 'True', '6', '6', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_EXPRESSPAYEPOS_SERVICEID', '4', '6', '7', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_EXPRESSPAYEPOS_CODE', '123', '6', '8', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_EXPRESSPAYEPOS_TOKEN', 'a75b74cbcfe446509e8ee874f421bd66', '6', '9', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_EXPRESSPAYEPOS_USESIGNATURE', 'True', '6', '10', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_EXPRESSPAYEPOS_SECRETWORD', 'sandbox.expresspay.by', '6', '11', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_EXPRESSPAYEPOS_NOTIFURL', '".$gg."', '6', '12', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_EXPRESSPAYEPOS_USESIGNATUREFORNOTIF', 'False', '6', '13', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_EXPRESSPAYEPOS_SECRETWORDFORNOTIF', '', '6', '14', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_EXPRESSPAYEPOS_ISNAMEEDIT', 'True', '6', '15', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_EXPRESSPAYEPOS_ISADRESSEDIT', 'True', '6', '16', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_EXPRESSPAYEPOS_ISAMOUNTEDIT', 'True', '6', '17', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
    }
    ...
    function keys() {
      return array('MODULE_PAYMENT_EXPRESSPAYEPOS_STATUS', 'MODULE_PAYMENT_EXPRESSPAYEPOS_ALLOWED', 
      'MODULE_PAYMENT_EXPRESSPAYEPOS_SORT_ORDER', 'MODULE_PAYMENT_EXPRESSPAYEPOS_ZONE',
      'MODULE_PAYMENT_EXPRESSPAYEPOS_ORDER_STATUS_ID', 'MODULE_PAYMENT_EXPRESSPAYEPOS_ISTEST', 
      'MODULE_PAYMENT_EXPRESSPAYEPOS_SERVICEID', 'MODULE_PAYMENT_EXPRESSPAYEPOS_CODE','MODULE_PAYMENT_EXPRESSPAYEPOS_TOKEN', 'MODULE_PAYMENT_EXPRESSPAYEPOS_USESIGNATURE',
      'MODULE_PAYMENT_EXPRESSPAYEPOS_SECRETWORD','MODULE_PAYMENT_EXPRESSPAYEPOS_NOTIFURL','MODULE_PAYMENT_EXPRESSPAYEPOS_USESIGNATUREFORNOTIF',
      'MODULE_PAYMENT_EXPRESSPAYEPOS_SECRETWORDFORNOTIF',
      'MODULE_PAYMENT_EXPRESSPAYEPOS_ISNAMEEDIT', 'MODULE_PAYMENT_EXPRESSPAYEPOS_ISADRESSEDIT', 'MODULE_PAYMENT_EXPRESSPAYEPOS_ISAMOUNTEDIT');
    }
